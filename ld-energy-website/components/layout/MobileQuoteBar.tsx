...
export function MobileQuoteBar(){
  ...
  useEffect(()=>{
    ...
    if(target)observer.observe(target);else setHeroVisible(false)
  ...
  return <div data-mobile-quote className="fixed inset-x-0 bottom-0 z-40 border-t border-secondary-200 bg-white/95 px-4 pt-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] shadow-lg backdrop-blur-sm md:hidden"><Link prefetch={false} href={href} className="flex min-h-12 items-center justify-center rounded-xl bg-[#386B59] px-4 py-3 text-base font-semibold text-white hover:bg-[#2E594B]">Get my exact quote</Link></div>
}
